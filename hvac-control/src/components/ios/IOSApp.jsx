...
import IOSDashboard from './IOSDashboard';
import IOSHistory from './IOSHistory';
  ...
  return (
    <div className="ios-app">
      <header className="ios-header">
        <div className="ios-header__content">
          <h1 className="ios-header__title">Home</h1>
          <div className="ios-header__actions">
            {!authenticated ? (
              <button onClick={handleSignIn} className="ios-btn ios-btn--primary">
                Sign In
              </button>
            ) : (
              <>
                <button
                  onClick={loadData}
                  className="ios-btn ios-btn--icon"
                  disabled={loading}
                  aria-label="Refresh"
                >
                  <svg className={`ios-icon ${loading ? 'ios-icon--spinning' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2"/>
                  </svg>
                </button>
                <button onClick={handleSignOut} className="ios-btn ios-btn--text">
                  Sign Out
                </button>
              </>
            )}
        ...
        <nav className="ios-tabs">
          <button
            className={`ios-tab ${activeView === 'dashboard' ? 'ios-tab--active' : ''}`}
            onClick={() => setActiveView('dashboard')}
          >
            <svg className="ios-tab__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <rect x="3" y="3" width="7" height="7"/>
              <rect x="14" y="3" width="7" height="7"/>
              <rect x="14" y="14" width="7" height="7"/>
              <rect x="3" y="14" width="7" height="7"/>
            </svg>
            <span className="ios-tab__label">Dashboard</span>
          </button>
          <button
            className={`ios-tab ${activeView === 'history' ? 'ios-tab--active' : ''}`}
          ...
          >
            ...
            <span className="ios-tab__label">History</span>
          </button>
        </nav>
      </header>
