...
import VictorianDashboard from './VictorianDashboard';
import VictorianHistory from './VictorianHistory';
import { fetchPanelStatus, fetchLogHistory, fetchLightStatus, fetchLockStatus, fetchPlugStatus, updateControl, toggleLight, toggleLock, triggerHvacWebhook, togglePlug, MOCK_PANEL_DATA, LIGHT_ONLY_ZONES } from '../../services/sheets';
import { initializeAuth, signIn, signOut, isAuthenticated } from '../../services/auth';
import './VictorianApp.css';

function VictorianApp() {
  ...
  const handleUpdateZone = async (zoneId, settings) => {
    ...
    const result = { sheetUpdated: false, webhookTriggered: false };
    ...
    try {
      ...
      if (settings.power !== undefined) await updateControl(zoneIndex, 'power', settings.power);
      if (settings.mode !== undefined) await updateControl(zoneIndex, 'mode', settings.mode);
      if (settings.target !== undefined) await updateControl(zoneIndex, 'target', settings.target);
      ...
      if (webhookResult.success) result.webhookTriggered = true;
      setZones(updatedZones);
      return result;
    } catch (error) {
  ...
  const handleRestoreDefault = async (zoneId) => {
    ...
    if (!defaultState) throw new Error('No default state available');
    ...
    try {
      ...
      if (defaultState.target) await updateControl(zoneIndex, 'target', defaultState.target);
      return { success: true };
    } catch (error) {
  ...
  const handleToggleLight = async (lightRow, lightName) => {
    ...
    updatedLights[lightIndex] = {
      ...currentLight,
      pendingChange: { state: newState, requestedAt: new Date() },
    };
    setLights(updatedLights);
    try {
  ...
  return (
    <div className="victorian-app">
      <header className="victorian-header">
        <div className="victorian-header__content">
          <h1 className="victorian-header__title">Domicile</h1>
          <div className="victorian-header__actions">
            {!authenticated ? (
              <button onClick={handleSignIn} className="victorian-btn victorian-btn--primary">
                Sign In
              </button>
            ) : (
              <>
                <button
                  ...
                  aria-label="Refresh"
                >
                  <svg className={`victorian-icon ${loading ? 'victorian-icon--spinning' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor">
                    <path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2"/>
                  </svg>
                </button>
                <button onClick={handleSignOut} className="victorian-btn">
                  Sign Out
                </button>
              </>
            )}
      ...
      <nav className="victorian-nav">
        <div className="victorian-nav__inner">
          <button
            className={`victorian-tab ${activeView === 'dashboard' ? 'victorian-tab--active' : ''}`}
            onClick={() => setActiveView('dashboard')}
          >
            Chambers
          </button>
          <button
            className={`victorian-tab ${activeView === 'history' ? 'victorian-tab--active' : ''}`}
