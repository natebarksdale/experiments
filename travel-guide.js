...
const AppState = {
    ...
    markers: [],
    isGenerating: false,
    generationId: 0, // Increments per search so late responses from an old search are ignored
...
async function loadPlace(location, addToHistory = true) {
    const generationId = ++AppState.generationId;
    ...
    // Each category is generated by its own request, in parallel, and rendered the
    // moment it lands. Output tokens generate sequentially per request, so five small
    // requests finish in roughly a fifth of the time of one big one.
    const locationType = detectLocationType(location);
    const categoryNames = CATEGORY_TEMPLATES[locationType] || CATEGORY_TEMPLATES.default;
    const placeData = {
        name: location,
        type: locationType,
        coordinates: null,
        categories: new Array(categoryNames.length).fill(null),
        mentionedPlaces: []
    };
    ...
    // Placeholder sections keep the categories in a stable order as they fill in
    const slots = categoryNames.map(name => {
        const el = createPendingCategoryElement(name);
        container.appendChild(el);
        return el;
    });

    const isStale = () => generationId !== AppState.generationId;
    let firstArrived = false;
    const failures = [];

    const outcomes = await Promise.all(categoryNames.map(async (categoryName, index) => {
        try {
            let category;
            try {
                category = await generateCategoryContent(location, locationType, categoryName);
            } catch (error) {
                // A garbled or empty answer is usually a one-off; give the section a second try
                if (isStale() || !/parse|empty response/i.test(error.message || '')) throw error;
                console.warn(`Retrying "${categoryName}" after: ${error.message}`);
                category = await generateCategoryContent(location, locationType, categoryName);
            }
            if (isStale()) return false;

            placeData.categories[index] = category;
            if (!placeData.coordinates && category.coordinates) {
                placeData.coordinates = category.coordinates;
                if (category.type) placeData.type = category.type;
                document.getElementById('placeType').textContent = placeData.type || 'Location';
                showPlaceMaps(placeData);
            }
            if (!firstArrived) {
                firstArrived = true;
                document.getElementById('loadingState').style.display = 'none';
            }
            slots[index].replaceWith(createCategoryElement(category, index));
            return true;
        } catch (error) {
            if (isStale()) return false;
            console.error(`Error generating "${categoryName}":`, error);
            failures.push(error);
            slots[index].remove();
            return false;
        }
    ...
    AppState.isGenerating = false;
    document.getElementById('loadingState').style.display = 'none';
    if (document.getElementById('placeType').textContent === 'Loading...') {
        document.getElementById('placeType').textContent = placeData.type || 'Location';
    }

    const succeeded = outcomes.filter(Boolean).length;
    if (succeeded === 0) {
        showGenerationFailure(failures[0] || new Error('Failed to generate content'));
    } else if (failures.length > 0) {
        showNotification(`${failures.length} section${failures.length > 1 ? 's' : ''} failed to generate. ${describeGenerationError(failures[0])}`, 'error');
    }
}

// Placeholder category shown while its request is in flight
function createPendingCategoryElement(name) {
    const section = document.createElement('div');
    section.className = 'category pending';
    ...
    for (let i = 0; i < 3; i++) {
        ...
        item.className = 'item item-skeleton';
        item.innerHTML = '<div class="skeleton-line"></div><div class="skeleton-line short"></div>';
...
// Show the bottom map and the main map once we know where the place is
function showPlaceMaps(placeData) {
...
// Turn an error into the message we show users, with actionable suggestions
function describeGenerationError(error) {
    let errorMessage = (error && error.message) || 'Failed to generate content';
...
function showGenerationFailure(error) {
    const errorMessage = describeGenerationError(error);
...
function detectLocationType(location) {
    // Simple heuristic so we don't spend an API call classifying the place
    const lowerLocation = location.toLowerCase();
    if (lowerLocation.includes('museum')) return 'museum';
    if (lowerLocation.includes('cathedral') || lowerLocation.includes('tower') ||
        lowerLocation.includes('palace') || lowerLocation.includes('castle')) return 'building';
    if (lowerLocation.includes('monument') || lowerLocation.includes('statue') ||
        lowerLocation.includes('memorial')) return 'monument';
    if (lowerLocation.includes('park') || lowerLocation.includes('beach') ||
        lowerLocation.includes('mountain') || lowerLocation.includes('forest')) return 'nature';
    if (lowerLocation.includes('restaurant') || lowerLocation.includes('cafe') ||
        lowerLocation.includes('bar')) return 'restaurant';
    if (location.split(' ').length <= 2 && !lowerLocation.includes('the')) return 'city';
    return 'default';
}

function buildYearContext() {
    if (AppState.selectedYear === 2025) return '';
    const yearDisplay = formatYearDisplay(AppState.selectedYear);
    if (AppState.selectedYear > 2025) {
        return ` Write as if visiting in ${yearDisplay}. Speculate on future developments.`;
    } else if (AppState.selectedYear > 0) {
        return ` Write about ${yearDisplay}. Describe what actually existed then.`;
    }
    return ` Write about ${yearDisplay} (ancient/prehistoric era). Describe the landscape and what existed.`;
}

// Generate one category (3 items: 2 truths, 1 lie) for a place
async function generateCategoryContent(location, locationType, categoryName) {
    const writingStyle = WRITING_STYLES[AppState.writingStyle]
        || AppState.customVoices[AppState.writingStyle]?.style
        || WRITING_STYLES.parker;
...
Return only valid JSON, no prose before or after:
{
  ...
  "items": [
    {"text": "True fact with <strong>Place Name</strong> and <strong>details</strong>.", "isLie": false},
    {"text": "False fact with <strong>Landmark</strong> and <strong>feature</strong>.", "isLie": true, "explanation": "Short reason (10 words max)"},
    {"text": "Another true fact with <strong>Restaurant</strong> and <strong>dish</strong>.", "isLie": false}
  ]
}`;

    const response = await callLLM(prompt, 1200);

    const data = extractJsonObject(response, 'items');
    if (!data || !Array.isArray(data.items) || data.items.length < 3) {
        console.error(`Failed to parse "${categoryName}" from response:`, response.slice(0, 500));
    ...
    // Fisher-Yates shuffle so the lie isn't always in the same position
    ...
    return {
        ...
        coordinates: hasCoords ? { lat: Number(coords.lat), lon: Number(coords.lon) } : null
    };
}

const LLM_TIMEOUT_MS = 90000; // Abort a request that hangs instead of spinning forever

// Reasoning models (gpt-oss, Mercury, etc.) spend output tokens on hidden thinking,
// which on OpenRouter counts against max_tokens and slows everything down. This guide
// doesn't need it, so we try to turn it off; models that can't disable it get the
// lowest effort, and models that reject the parameter outright get none. The setting
// that worked is remembered per model so the retry only happens once.
...
function getReasoningModeIndex(model) {
    try {
        const stored = parseInt(localStorage.getItem(`travel_guide_reasoning_mode:${model}`));
        if (Number.isInteger(stored) && stored >= 0 && stored < REASONING_MODES.length) return stored;
    } catch (e) { /* ignore */ }
    return 0;
}

function setReasoningModeIndex(model, index) {
...
async function callLLM(prompt, maxTokens = 2000) {
    ...
    let modeIndex = getReasoningModeIndex(model);
    for (;;) {
        try {
...
async function callLLMOnce(prompt, maxTokens, reasoning) {
    ...
    try {
        ...
        const body = {
            ...
            // Route to whichever host serves this model fastest (e.g. Cerebras or Groq
            // for open-weight models) instead of OpenRouter's default price-first routing
...
// Pull a JSON object carrying `requiredKey` out of a model response. Tolerates <think>
// blocks, markdown code fences, chatty preambles containing braces, and output cut off
// at the token cap.
function extractJsonObject(response, requiredKey) {
    let text = response
        .replace(/<think>[\s\S]*?<\/think>/gi, '')
        .replace(/```(?:json)?/gi, '');

    // Prefer the object that actually contains our schema
    const schemaIdx = text.indexOf(`"${requiredKey}"`);
    const candidates = [];
    ...
    for (const start of candidates) {
        try {
            ...
            if (parsed && typeof parsed === 'object' && parsed[requiredKey] !== undefined) return parsed;
        } catch (e) {
    ...
    for (const start of candidates) {
        ...
        if (repaired && repaired[requiredKey] !== undefined) {
...
function checkPageCompletion() {
    if (AppState.isGenerating) return; // More sections are still on their way
    const allItems = document.querySelectorAll('.item:not(.item-skeleton)');
